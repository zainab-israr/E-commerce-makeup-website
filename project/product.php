...
@$db = new mysqli('localhost', $username, $password,$dsn);
 ...
 $query = "SELECT PCat,PTittle, PHTittle
 FROM product WHERE PID='$id'";
 $stmt = $db->prepare($query);
  $stmt->execute();
 $stmt->store_result();
 $stmt->bind_result($cat,$pt,$pht);
 while($stmt->fetch()) {
 $tittle=$pht;
 $heading=$pt;
 $Cat=$cat;
...
<?php 
   ...
   else if(($Cat=="LIP BALMS & TREATMENTS"))
   {
    ...
    <li class="breadcrumb-item" >
      <?php
           if($Cat=="Lip Balms & Treatments")
                $Cat="LIP BALMS";
            echo "<a href='category.php?cat=$Cat' id='b'>"
                ?><span class="clr">
                  <?php
                   if($Cat=="LIP BALMS")
                   $Cat="Lip Balms & Treatments";   
...
@$db = new mysqli('localhost', $username, $password,$dsn);
 ...
 $query = "SELECT PID,PPic, PName,PPrice, Images,Details,Uses
 FROM product WHERE PID='$id'";
 $stmt = $db->prepare($query);
  $stmt->execute();
 $stmt->store_result();
 $stmt->bind_result($pid,$pic, $pname, $price,$images,$d,$u);
 ?>
...
while($stmt->fetch()) {
  ...
  <div class="carousel-inner">
    <div class="carousel-item active">
      <img src="<?php echo $pic ?>" style="width:400px;height:500px;margin-left:150px;" class="d-block" alt="...">
    </div>
    <?php
    if($images!="empty")
  {
    ...
    <div class="carousel-item">
      <img src="<?php echo $value ?>"  style="width:400px;height:500px;margin-left:150px;" class="d-block" alt="...">
    </div>
    <?php
    }
    ?>
...
<form <?php echo "action='addtocart.php?PID=$pid' method='post'" ?>>
   <div class="row">
  <div class="col-sm-1" style="margin-top:10px;">
    <select class="o" id="Quantity" name="Quantity">
  ...
  <input type="submit" class="button heading3" value="Add to Bag">
</div>
  </div>
  </form> 
  <div style="margin-top:30px;margin-right:40px;"> 
    <?php
    echo "<p class='details' style='margin-top: 0px;'>Details</p>";
    echo "<p class='details2' style='margin-top: 0px;'>$d</p>";
    echo "<p class='details' style='margin-top: 5px;'>How to use</p>";
    if(strpos($u,'.')==0)
       echo "<p class='details2' style='margin-top: 0px;'><ul><li>$u</li></ul></p>";
    else
       {
         $p=strpos($u,'.');
         $arr = explode(".", $u);
         echo "<p class='details2' style='margin-top: 0px;'><ul>";
         foreach($arr as $value)
        {
           echo "<li>$value</li>";
        }
